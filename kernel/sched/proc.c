  ...
  for (;;) {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
      ...
      if (p->gas_remaining == 0) {
        p->out_of_gas = 1; // Mark as out of gas
        continue; // Skip this process
      }
      // Deduct gas for this scheduling quantum (simplified as per tick)
      if (p->gas_remaining <= GAS_PER_TICK) {
          p->gas_remaining = 0;
          p->out_of_gas = 1;
      } else {
          p->gas_remaining -= GAS_PER_TICK;
      }
