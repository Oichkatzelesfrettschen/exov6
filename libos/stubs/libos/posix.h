/**
 * @file posix.h
 * @brief Host-backed libos POSIX API declarations
 *
 * This header provides declarations for the libos POSIX API
 * when using host syscalls (for demos running on development hosts).
 */
#pragma once
...
/* libos signal set type */
typedef unsigned long libos_sigset_t;

/* Exokernel capability type (stub for host mode) */
...
/* IPC (stubs - not available on host) */
