...
#define PGSIZE 4096
#include "src/engine/include/iommu.h"
...
#include "src/engine/kernel/iommu.c"
...
def compile_and_run():
    with tempfile.TemporaryDirectory() as td:
        ...
        (pathlib.Path(td)/"spinlock.h").write_text(
            '#include "src/engine/include/types.h"\n'
            '#include "src/engine/kernel/include/spinlock.h"\n')
        (pathlib.Path(td)/"defs.h").write_text("")
        (pathlib.Path(td)/"mmu.h").write_text('#include "src/engine/include/types.h"\n#include "src/engine/include/mmu.h"\n')
        (pathlib.Path(td)/"memlayout.h").write_text('#include "src/engine/include/memlayout.h"\n')
        (pathlib.Path(td)/"types.h").write_text('#include "src/engine/include/types.h"\n')
